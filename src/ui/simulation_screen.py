...
class SimulationScreen:
    def __init__(self):
        ...
        self.restart_button = pygame.Rect(300, 440, 200, 50)

        self.controls =  Controls(self.screen, self.start_button, self.quit_button, self.resume_button, self.restart_button)

        self.background_image = pygame.image.load("src/ui/images/noise.png").convert()
        self.background_image = pygame.transform.scale(self.background_image, (self.SCREEN_WIDTH, self.SCREEN_HEIGHT))

        self.resources_generator = ResourceGemerator(self.screen, 4)
        self.resources_generator.generate_resources()

        self.population_dots = PopulationDots(10)   
    ...
    def run(self):
        ...
        pause_screen = PauseScreen(self.screen, self.resume_button, self.quit_button, self.restart_button)

        while running:
            game_running, paused, restart = self.controls.handle_events(game_running, paused, restart, self.population_dots)
